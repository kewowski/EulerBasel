/-
EulerBasel/SegmentIntegralPunctured.lean

Analytic helper lemmas for punctured neighbourhoods:
segment integral along s ↦ (s : ℂ) * z preserves convergence to 0.

This is used by CoefficientProductSide_QuadraticBridge.lean.
-/
...
lemma tendsto_segmentIntegral_comp_mul_punctured (r : ℂ → ℂ)
    (hr : Tendsto r (𝓝[≠] (0 : ℂ)) (𝓝 (0 : ℂ))) :
    Tendsto (fun z : ℂ => ∫ s : ℝ in (0 : ℝ)..1, r ((s : ℂ) * z))
      (𝓝[≠] (0 : ℂ))
      (𝓝 (0 : ℂ)) := by
  classical
  ...
  have hmain :
      ∀ᶠ z : ℂ in 𝓝[≠] (0 : ℂ),
        ‖(∫ s : ℝ in (0 : ℝ)..1, r ((s : ℂ) * z))‖ < ε := by
    filter_upwards [hne, hzball] with z hzne hzρ

    have hbound :
        ∀ s : ℝ, s ∈ (Ι (0 : ℝ) (1 : ℝ)) → ‖r ((s : ℂ) * z)‖ ≤ (ε / 2 : ℝ) := by
      intro s hsI
      ...
      have hzlt : ‖z‖ < ρ := hzρ
      have hszlt : ‖(s : ℂ) * z‖ < ρ := lt_of_le_of_lt hnorm_mul hzlt
    ...
    have hle' :
        ‖(∫ s : ℝ in (0 : ℝ)..1, r ((s : ℂ) * z))‖
          ≤ (ε / 2 : ℝ) * |(1 : ℝ) - (0 : ℝ)| := by
      simpa using
        (intervalIntegral.norm_integral_le_of_norm_le_const
          (a := (0 : ℝ)) (b := (1 : ℝ)) (C := (ε / 2 : ℝ))
          (f := fun s : ℝ => r ((s : ℂ) * z)) hbound)

    have hle : ‖(∫ s : ℝ in (0 : ℝ)..1, r ((s : ℂ) * z))‖ ≤ (ε / 2 : ℝ) := by
  ...
  have :
      ∀ᶠ x : ℂ in 𝓝[≠] (0 : ℂ),
        dist (∫ s : ℝ in (0 : ℝ)..1, r ((s : ℂ) * x)) (0 : ℂ) < ε := by
    refine hmain.mono ?_
    intro x hx
